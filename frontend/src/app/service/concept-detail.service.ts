...
export class ConceptDetailService {
  ...
  getConcepts(terminology: string, codes: string, include: string): Observable<any> {
    return this.http.get(encodeURI('/api/v1/concept/' + terminology.toLowerCase() + '?include=' + include + '&list=' + codes)).pipe(
      catchError((error) => {
  ...
  getConceptSummary(conceptCode: string, include: string, limit: number = null): Observable<any> {
    return this.http
      .get(
        encodeURI(
          '/api/v1/concept/' +
            this.configService.getTerminology().terminology +
            '/' +
            conceptCode +
            '?include=' +
            include +
  ...
  getProperties(): Observable<any> {
    return this.http
      .get(encodeURI('/api/v1/metadata/' + this.configService.getTerminologyName() + '/properties'), {
        responseType: 'json',
      })
      .pipe(
        catchError((error) => {
  ...
  getHierarchyData(code: string, limit: number = null) {
    const url = '/api/v1/concept/' + this.configService.getTerminologyName() + '/' + code + '/subtree' + (limit ? '?limit=' + limit : '');
    return this.http
      .get(encodeURI(url))
      .toPromise()
  ...
  getHierarchyChildData(code: string, limit: number = null) {
    const url = '/api/v1/concept/' + this.configService.getTerminologyName() + '/' + code + '/subtree/children' + (limit ? '?limit=' + limit : '');
    return this.http
      .get(encodeURI(url))
      .toPromise()
      .then((res) => <TreeNode[]>res);
